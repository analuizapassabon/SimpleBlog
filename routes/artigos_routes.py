...
async def post_cadastrar(
    ...
    logger.info(f"POST /artigos/cadastrar - Usuário: {usuario_logado['id']}")
    logger.info(f"Dados recebidos - Título: {titulo[:50] if titulo else 'vazio'}, Status: {status_artigo}, Categoria: {categoria_id}")
    ...
    try:
        ...
        logger.info(f"DTO validado com sucesso para artigo: {dto.titulo}")

        # Verifica se título já existe
        if artigo_repo.titulo_existe(dto.titulo):
            logger.warning(f"Título já existe: {dto.titulo}")
        ...
        logger.info(f"Tentando inserir artigo no banco de dados...")
        artigo_id = artigo_repo.inserir(novo_artigo)

        if artigo_id:
            logger.info(f"Artigo '{dto.titulo}' criado por usuário {usuario_logado['id']} - ID: {artigo_id}")
            ...
            logger.error(f"Falha ao inserir artigo no banco - artigo_repo.inserir retornou None")
        ...
        logger.error(f"Erro de validação ao criar artigo: {e}")
        ...
        logger.error(f"Erro inesperado ao criar artigo: {e}", exc_info=True)
        informar_erro(request, f"Erro ao cadastrar artigo: {str(e)}")
        return RedirectResponse(
            url="/artigos/cadastrar",
            status_code=status.HTTP_303_SEE_OTHER,
        )
